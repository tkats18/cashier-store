...
from cashier import IPaymentConfirm, IReceiptFiller, IReceiptOpener
from customer import IPayer, IProductChooser
...
class CustomerChooserHandler(Handler):
    def __init__(
        self, cur_customer: IProductChooser, next_handler: Optional["Handler"] = None
    ):
...
class CashierReceiptOpenHandler(Handler):
    def __init__(
        self, cur_cashier: IReceiptOpener, next_handler: Optional["Handler"] = None
    ):
        super().__init__(next_handler)
...
class CashierReceiptFillHandler(Handler):
    def __init__(
        self, cur_cashier: IReceiptFiller, next_handler: Optional["Handler"] = None
    ):
        super().__init__(next_handler)
        self.cur_cashier = cur_cashier
...
class CashierReceiptPaymentAcceptHandler(Handler):
    def __init__(
        self, cur_cashier: IPaymentConfirm, next_handler: Optional["Handler"] = None
    ):
        super().__init__(next_handler)
        self.cur_cashier = cur_cashier

    def handle(self, req: Request) -> None:
        self.cur_cashier.confirm_payment(req.receipt, req.paid_amount)
        super().handle(req)


class ManagerReportGenerateHandler(Handler):
    def __init__(
        self, cur_manager: IReportGenerator, next_handler: Optional["Handler"] = None
    ):
        super().__init__(next_handler)
    ...
    def handle(self, req: Request) -> None:
        if req.customer_num % 20 == 0:
            self.cur_cashier.generate_report_by_desire()
            super().handle(req)


class ManagerShiftEndHandler(Handler):
    def __init__(
        self, cur_manager: IShiftEnder, next_handler: Optional["Handler"] = None
    ):
        super().__init__(next_handler)
        self.cur_cashier = cur_manager
    ...
    def handle(self, req: Request) -> None:
        if req.customer_num % 100 == 0:
            self.cur_cashier.end_shift_by_desire()
            super().handle(req)
