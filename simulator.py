...
from simulator_chain import CustomerChooserHandler, CashierReceiptOpenHandler, CustomerPaymentHandler, \
    CashierReceiptFillHandler, CashierReceiptPaymentAcceptHandler, Request, ManagerReportGenerateHandler, \
    ManagerShiftEndHandler
...
def simulate_single_cashier_single_customer():
    ...
    manager = StoreManager(storage, shop)
    customer_num = 0
    ...
    while shop.shift < 3:
        ...
        shift_end = ManagerShiftEndHandler(cur_manager=manager, next_handler=report_generate)
        payment_accept = CashierReceiptPaymentAcceptHandler(cashier, next_handler=shift_end)
